...
contract BondingManager is ManagerProxyTarget, IBondingManager {
    ...
    mapping(bytes32 => UnbondingLock) public unbondingLocks;
    uint256 lastUnbondingLockID;
    ...
    modifier autoClaimFees(address _delegate) {
        _claimFees(_delegate, msg.sender);
        _;
    }
    ...
    function bondWithHint(
        ...
        uint256 oldTotalStake = newPool.totalStake();
    ...
    function unbondWithHint(
        ...
        require(delegatorStatus(msg.sender) == DelegatorStatus.Bonded, "CALLER_NOT_BONDED");
        require(_amount > 0, "ZERO_UNBOND_AMOUNT");

        uint256 delegatorStake = stakeOf(msg.sender);
        ...
        bytes32 id = lastUnbondingLockID;
        lastUnbondingLockID++;
    ...
    function rebondWithHint(
        ...
        require(delegatorStatus(msg.sender) != DelegatorStatus.Unbonded, "CALLER_NOT_UNBONDED");
    ...
    function rebondFromUnbondedWithHint(
        ...
        unbondingLocks[_unbondingLockId].orchestrator = _to;
        // Process rebond using unbonding lock
        _processRebond(msg.sender, _unbondingLockId, _newPosPrev, _newPosNext);
    }
    ...
    function transcoderWithHint(
        ...
        if (!orchestratorPoolV2.contains(msg.sender)) {
            _tryToJoinActiveSet(msg.Sender, o.delegationPool.totalStake(), currentRound + 1, _newPosPrev, _newPosNext);
        }
        ...
        currentRoundInitialized
    {
        ...
        require(isActiveOrchestrator(msg.sender), "caller must be an active transcoder");
        ...
        uint256 totalStake = o.delegationPool.totalStake();
        uint256 rewardTokens = minter().createReward(totalStake, currentRoundTotalActiveStake);

        _updateOrchestratorWithRewards(msg.sender, rewardTokens, currentRound, _newPosPrev, _newPosNext);
    ...
    function orchestratorTotalStake(address _orchestrator) public view returns (uint256) {
        return orchestrators[_orchestrator].delegationPool.totalStake();
    }
     ...
     */
    function isActiveOrchestrator(address _orchestrator) public view returns (bool) {
     ...
     */
    function getTranscoderPoolSize() public override view returns (uint256) {
     ...
     */
    function getTotalBonded() public override view returns (uint256) {
    ...
    function _resignOrchestrator(address _orchestrator) internal {
        ...
        nextRoundTotalActiveStake = nextRoundTotalActiveStake.sub(orchestratorTotalStake(_orchestrator));
    ...
    function _tryToJoinActiveSet(
        ...
        if (orchestratorPoolV2.isFull()) {
            ...
            uint256 lastStake = orchestrators[lastOrchestrator].delegationPool.totalStake();
    ...
    function _processRebond(
        ...
        uint256 amount = lock.amount;
        uint256 delegate = lock.orchestrator;

        // Claim outstanding fees
        _claimFees(delegate, _delegator);

        // Increase delegator's bonded amount
        uint256 oldStake = stakeOf(_delegator);
